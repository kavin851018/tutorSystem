<script>window.location = '../booking/';</script>
<?php
session_start();
@$m = $_GET['m'];
@$d = $_GET['d'];
...
$timePeriod = array("13:00~14:00", "14:00~15:00","15:00~16:00", "16:00~17:00", "17:00~18:00",  "18:00~19:00", "19:00~20:00", "20:00~21:00"); // Each time period
$periodNumber = array(128, 64, 32, 16, 8, 4, 2, 1);

for ($i=0; $i<count($timePeriod); $i++) // each time {9:00 ~ 10:00 癒K}
{
...
?>
     ...
     <p align="center" class="content">若你無法自行取消預約，請與自學園連絡: 07-5252000 ext.3157 或 onlinelearning@mail.nsysu.edu.tw</p>
</body>
</html>
<?php
mysql_close($conn);
...
function printBookField($year, $day, $periodNumber, $tutor, $location, $conn)
{
	...
	//{
		$tutor_leave = mysql_query("SELECT * FROM `tutor_leave` WHERE `date`='$d_dash' AND `periodNumber`='$periodNumber'", $conn);
		$get_tl = mysql_fetch_array($tutor_leave); //Get tutor leave record

		$bookedOrder = $i + 1;
		echo "<td align=center nowrap>";
		//echo "<font size=2 color=gray>" . $timeSlot[$i] . "</font>";
		...
		else // Bookable, Overdue, or Already Booked
		{
			...
			$sql = "Select * From booked Where location='$location' AND booked_date='$d' AND tutor='$tutor' ";
			...
			if( $n == 0 ) // Bookable or Overdue
			{
				...
				if($d_dash==$get_tl['date'] && $periodNumber==$get_tl['periodNumber']){ //Tutor take leave
					echo "小老師請假<br><img src=images/notyet.png border=0 alt=\"Not Yet!\">";
				}
				elseif( $dothing==0 ) // Overdue      source: $this_d < $_SESSION['overdue']
			...
			else // Already Booked
			{
				...
				if($d_dash==$get_tl['date'] && $periodNumber==$get_tl['periodNumber']){ //Tutor take leave
					echo "<br>小老師請假<br><img src=images/notyet.png border=0 alt=\"Not Yet!\">";
				}
				elseif($dothing==1){
					...
					echo "<br><img src=images/overdue.png border=0 alt=\"Overdue\">";
				}
