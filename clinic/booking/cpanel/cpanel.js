...
function create_tutor(){	
    ...
    $('.ui.button.tutor-edit').click(function(e){
    	$(this).hide();//隱藏修改button
    	$(this).next().show();//顯示完成button
    	$(this).parent().parent().find('input').show();
    	$(this).parent().parent().find('.dropdown').show();
    	$(this).parent().parent().find('.content').hide();
    	$(this).next().next().hide();//隱藏刪除button
    	$(this).next().next().next().show();//顯示取消button
    })

    $('.ui.button.tutor-cancel').click(function(e){
    	$(this).hide();//隱藏取消button
    	$(this).prev().prev().prev().show();//秀出修改button
    	$(this).prev().show();//秀出刪除button
    	$(this).prev().prev().hide();//隱藏完成button
      	$(this).parent().parent().find('input').hide();
    	$(this).parent().parent().find('.dropdown').hide();
    	$(this).parent().parent().find('.content').show();
    })
    $('.ui.button.tutor-complete').click(function(e){
    		var completeButton=$(this);
    		var form=$(this).parent().parent();
    		var tutor_name=form.find('input[name="name"]');
    		var tutor_id=form.find('input[name="id"]');
    		var tutor_dept=form.find('input[name="dept"]');
    		var tutor_phone=form.find('input[name="phone"]');
    		var tutor_email=form.find('input[name="email"]');
			var rno = $(this).data('rno');
	    	$.ajax({
				type: 'POST',
	        	url: 'cpanel_update.php',
				data: {'type':'edit_tutor','rno':rno,'name':tutor_name.val(),'id':tutor_id.val(),'dept':tutor_dept.val(),'phone':tutor_phone.val(),'email':tutor_email.val()},
				success: function(data){
					alert(data.status);
					...
					tutor_name.hide();
					tutor_id.hide();
					form.find('.dropdown').hide();
					tutor_phone.hide();
					tutor_email.hide();   	

					form.find('.content.name').text(tutor_name.val());
					form.find('.content.id').text(tutor_id.val());
					form.find('.content.dept').text(tutor_dept.val());
					form.find('.content.phone').text(tutor_phone.val());
					form.find('.content.email').text(tutor_email.val());

					form.find('.content.name').show();
					form.find('.content.id').show();
					form.find('.content.dept').show();
					form.find('.content.phone').show();
					form.find('.content.email').show();

					completeButton.hide();
					completeButton.next().show();	
					completeButton.next().next().hide();
					completeButton.prev().show();
